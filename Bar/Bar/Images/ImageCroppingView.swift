...
//  Bar
//
//  Created by David Chen on 3/24/21. (Note: completely poached from online)
//  Source: Created by Christopher Guirguis on 12/17/20.
//
        ...
        ZStack{
            ...
            VStack {
                ...
                HStack (alignment: .top) {
                    ...
                    Spacer()
                    Button(action: {
                        let cgImage: CGImage = self.image.upOrientationImage().cgImage!
                        ...
                        if let cImage = cgImage.cropping(
                            to: CGRect(x: xOffset, y: yOffset, width: scaledDim, height: scaledDim)
                        ){
                            self.croppedImage = UIImage(cgImage: cImage)
                            self.profileVM.writeImageToFirebase(image: self.croppedImage, isProfPic: self.isProfPic, orderIdx: self.orderIdx)
                            self.shown = false
                        }
                    }){
                        Text("Done")
                ...
                Spacer()
                ZStack{
                    Image(uiImage: self.image.upOrientationImage())
                        .resizable()
                        .scaledToFit()
